module.exports = {
 async activeCurrenciesDetailsByOrg(orgId: any) {
  ...
  async activeLeaderBoardsByUser(userId: any){
  ...
  async activeCompitionsByLeaderboard(leaderboards: any) {
    const leaderboardsIds = leaderboards.map((leaderboard: any) => leaderboard._id);
    var filter = {
  ...
  async subscriptionWithProduct(organizationId: any) {
