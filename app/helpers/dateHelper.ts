...
module.exports = {
  ...
  convertTimeIntoSpecificFormate: function (
    queryTime: any,
    inputFormate: any,
    outputFormate: any
  ) {
  ...
  convertTimeIntoSpecificFormate2: function (
    queryTime: any,
    outputFormate: any
  ) {
    var count = 0;
  ...
  isAnOverlapEvent: function (
    ...
    if (
      moment(startTime).isSameOrAfter(queryStartTime) &&
      moment(endTime).isSameOrBefore(queryEndTime)
    ) {
      return true;
    }
    return false;
  ...
  getLastFiveWeek: function (date: any) {
    let dates = [];
    dates.push(
      moment(date).subtract("days", -1).format("YYYY-MM-DD") + "T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("days", 7).format("YYYY-MM-DD") + "T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("days", 14).format("YYYY-MM-DD") + "T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("days", 21).format("YYYY-MM-DD") + "T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("days", 28).format("YYYY-MM-DD") + "T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("days", 35).format("YYYY-MM-DD") + "T00:00:00.000Z"
    );
    return dates;
  },
  getLastFiveMonth: function (date: any) {
    let dates = [];
    dates.push(
      moment(date).add("month", 1).format("YYYY-MM") + "-01T00:00:00.000Z"
    );
    dates.push(
      moment(date).add("month", 0).format("YYYY-MM") + "-01T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("month", 1).format("YYYY-MM") + "-01T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("month", 2).format("YYYY-MM") + "-01T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("month", 3).format("YYYY-MM") + "-01T00:00:00.000Z"
    );
    dates.push(
      moment(date).subtract("month", 4).format("YYYY-MM") + "-01T00:00:00.000Z"
    );
    return dates;
  },
  getLast30Days: function () {
    ...
    for (let index = 0; index < 31; index++) {
      dates.push(
        moment().subtract("days", index).format("YYYY-MM-DD") + "T00:00:00.000Z"
      );
    }
    return dates;
  },
  getDesirePreviousMonthFromGivenDate: function (date: any, days: any) {
    return (
      moment(date).subtract(days, "month").format("YYYY-MM-DD") +
      "T00:00:00.000Z"
    );
  },
  ...
  diffInMinuts: function (endDate: any, startDate: any) {
    ...
    console.log(dt1, dt2, resultInMinutes);
