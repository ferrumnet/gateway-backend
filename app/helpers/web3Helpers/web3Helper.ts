...
let APELPCFRMBNBTokenContractAddress =
  "0x9aa0AB73409311984ED84f3Edef962201Bd11712";
let APELPCFRMxBNBTokenContractAddress =
  "0xb76b11410A506495418D20c58F9452c17CF285c1";
const MAX_AMOUNT =
...
module.exports = {
  ...
  async getTokenPriceFromRouter(currency: any, network: any) {
    try {
      ...
      let ApeContract = new web3.Contract(
        ApeRouterJson.abi as AbiItem[],
      ...
      if (currency.toLowerCase() === cFRMxTokenContractAddress.toLowerCase()) {
        ...
        currency.toLowerCase() === cFRMTokenContractAddress.toLowerCase()
      ) {
        pricingRoute = [
          currency,
      ...
      let response = await ApeContract.methods
        .getAmountsOut("1000000000000000000", pricingRoute)
        .call();

      if (response.length > 0) {
        return await swapUtilsHelper.amountToHuman(
  ...
  async getFeesUsingWeb3(network: any) {
    ...
    for (let item of tokens) {
      ...
      let priceDetails = await this.getTokenPriceFromRouter(
        item.currency,
        network
      );
  ...
  async approveToZero(
    ...
    let m = web3ConfigurationHelper
      .erc20(network.rpcUrl, cabn.tokenContractAddress)
      .methods.approve(contractAddress, "0");
  ...
  async approveMax(
    ...
    let m = web3ConfigurationHelper
      .erc20(network.rpcUrl, cabn.tokenContractAddress)
      .methods.approve(contractAddress, MAX_AMOUNT);
    let from = address.address;
    let gas = !!useThisGas
      ? Math.max(useThisGas, DEFAULT_APPROVE_GAS)
  ...
  async approve(
    ...
    contractAddress: string,
    useThisGas: number,
    rawAmount: typeof Big
  ) {
    console.log("about to approve: ", { amount: rawAmount.toFixed() });
    let m = web3ConfigurationHelper
      .erc20(network.rpcUrl, cabn.tokenContractAddress)
      .methods.approve(contractAddress, rawAmount.toFixed());
    let from = address.address;
    const gas = !!useThisGas
      ? Math.max(useThisGas, DEFAULT_APPROVE_GAS)
      : await m.estimateGas({ from });
    return [m.encodeABI(), gas];
  },
  ...
  async getUserBalance(network: any, cabn: any, address: any) {
    let balance = await web3ConfigurationHelper
      .erc20(network.rpcUrl, cabn.tokenContractAddress)
  ...
  async getAvaialableLiquidity(
    network: any,
    cabn: any,
    contractAddress: string
  ) {
    let web3Balance = await web3ConfigurationHelper
      .erc20(network.rpcUrl, cabn.tokenContractAddress)
      .methods.balanceOf(contractAddress)
      .call();
    let balance = await swapUtilsHelper.amountToHuman_(
      network,
      cabn,
      web3Balance
    );
