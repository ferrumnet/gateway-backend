...
function sendGridEmail(user, isFor = 'otp') {
  ...
  if (isFor == 'link') {
    ...
    postData = makeObjectBodyForOtp(user);
...
function makeObjectBodyForOtp(user) {
  ...
  personalizations.push({ to: to, dynamic_template_data: dynamic_template_data })
