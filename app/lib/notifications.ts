...
module.exports = {
  ...
  sendNotification: function (data: any) {
    let payload: any = {
      body: data.message,
  ...
  sendMultipleNotificationAndroid: function (data: any,tokens: any) {
  ...
  sendMultipleNotificationIOS: function (data: any,tokens: any) {
    if (!tokens || (tokens instanceof Array && tokens.length < 1)) {
      return Promise.resolve({ msg: {} });
    }
  ...
  sendCustomNotifications: function (registrationTokens: any, payloads: any) {

     admin.messaging().sendToDevice(registrationTokens, payloads, {
      priority: "high"
    }).then(function (firebaseRes: any) {
