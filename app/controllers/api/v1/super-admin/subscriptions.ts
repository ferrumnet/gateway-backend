...
module.exports = function (router: any) {
  router.post(
    ...
    asyncMiddleware(async (req: any, res: any) => {
      ...
      if (user && user.organization) {
        ...
        if (packageRes) {
          let subscriptionCount = await db.Subscription.countDocuments(
            subFilter
          );
    ...
    asyncMiddleware(async (req: any, res: any) => {
      ...
      if (user && user.organization) {
        ...
        if (packageRes) {
          ...
          let subscription = await db.Subscription.findOneAndUpdate(
            subfilter,
            req.body,
            { new: true }
          );
  ...
  router.put(
    ...
    asyncMiddleware(async (req: any, res: any) => {
      ...
      if (isValidObjectId(filter._id) && typeof payload.isActive == "boolean") {
        const subscription = await db.Subscription.findOneAndUpdate(
          filter,
          payload,
          { new: true }
        );
  ...
  router.get(
    ...
    asyncMiddleware(async (req: any, res: any) => {
      ...
      if (req.query.isPagination != null && req.query.isPagination == "false") {
        subscriptions = await db.Subscription.find(filter).sort({
          createdAt: -1,
        });
      } else {
  ...
  router.put(
    ...
    asyncMiddleware(async (req: any, res: any) => {
      const filter = { _id: req.params.id };
      const payload = {
        actualLimit: req.body.actualLimit,
