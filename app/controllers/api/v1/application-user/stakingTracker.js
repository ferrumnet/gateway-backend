...
module.exports = function (router) {
  ...
  router.post('/update/myStake', async (req, res) => {
    ...
    if(stakingParticipantsCount <  1){
      await stakingTrackerHelper.intiatParticipentsData(req.body.tokenContractAddress, req.body.stakingContractAddress )
    }

    const query = { stakingContractAddress: req.body.stakingContractAddress, stakeHolderWalletAddress: req.body.stakeHolderWalletAddress, tokenContractAddress: req.body.tokenContractAddress}
    ...
    const totalStakedAmount = stake? Number(stake.totalStakedAmount) + Number(stake.stakedAmount) :  Number(req.body.stakedAmount)
    const update = {$set: {stakedAmount: req.body.stakedAmount, rewardAmount: req.body.rewardAmount, totalStakedAmount, user: req.user._id,}}
