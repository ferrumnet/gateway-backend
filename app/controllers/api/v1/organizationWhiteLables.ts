module.exports = function (router: any) {
  router.get("/by/sitename/:siteName", async (req: any, res: any) => {
    ...
    let organizationWhiteLables = await db.OrganizationWhiteLables.findOne({
      ...
      .populate("leaderboardConfig.leaderboards");
  ...
  router.get("/menu/by/sitename/:siteName", async (req: any, res: any) => {
    ...
    organization = await db.Organizations.findOne({
    ...
    metaDataAndFilter.push({
      metaData: {
        $elemMatch: { key: "isCommunityMemberMenuItem", value: "true" },
      },
    });
    ...
    if (req.query.isForGateway) {
      if (req.query.isForGateway == "true") {
        metaDataAndFilter.push({
          metaData: { $elemMatch: { key: "isForGateway", value: "true" } },
        });
      }
    }
    ...
    if (req.query.isForCrucible) {
      if (req.query.isForCrucible == "true") {
        metaDataAndFilter.push({
          metaData: { $elemMatch: { key: "isForCrucible", value: "true" } },
        });
      }
    }
    ...
    let packageIds = await db.Subscription.find({
      organization: organization._id,
    }).distinct("package");
    let productIds = await db.Package.find({
      _id: { $in: packageIds },
    }).distinct("product");

    filter._id = { $in: productIds };
    let products = await db.Product.find(filter).sort(sort);

    await adjustMenuItems(
      products,
      req,
      organization,
      isFromOrganizationAdminPath
    );

    return res.http200({ menu: products });
  });

  async function adjustMenuItems(
    products: any,
    req: any,
    organization: any,
    isFromOrganizationAdminPath: boolean
  ) {
    if (products && products.length > 0) {
      for (let i = 0; i < products.length; i++) {
        ...
        if (product.tags && product.tags.includes("#leaderboards")) {
          product.menuItems.push(
            ...(await findLeaderboardsForMenu(
              product,
              organization,
              req,
              isFromOrganizationAdminPath
  ...
  async function findLeaderboardsForMenu(
    product: any,
    organization: any,
    req: any,
    isFromOrganizationAdminPath: boolean
    ...
    let data = await db.Leaderboards.find(filter).sort({ createdAt: 1 });

    if (data && data.length > 0) {
      for (let i = 0; i < data.length; i++) {
        let item = data[i];
        let path = "";
        path = getItemFromMetaData(
          product,
          "internalPubDynamicPathForSingle"
        ).value;

        if (item.type && item.type == "stake") {
          path = getItemFromMetaData(
            product,
            "internalPubDynamicPathForStake"
          ).value;
        } else if (item.leaderboardCurrencyAddressesByNetwork.length > 1) {
          path = getItemFromMetaData(
            product,
            "internalPubDynamicPathForMulti"
          ).value;
        }
        items.push({
          _id: item._id,
          name: item.name,
          isDynamic: true,
          count: item.leaderboardCurrencyAddressesByNetwork.length,
          path: path + item._id,
          icon: "",
        });
      }
  ...
  async function findCompetitionsForMenu(
    product: any,
    organization: any,
    req: any,
    isFromOrganizationAdminPath: boolean
    ...
    if (data && data.length > 0) {
      for (let i = 0; i < data.length; i++) {
        ...
        if (
          item &&
          item.leaderboard &&
          item.leaderboard.leaderboardCurrencyAddressesByNetwork &&
          ...
          item.leaderboard.leaderboardCurrencyAddressesByNetwork[0]
            .currencyAddressesByNetwork.currency
        ) {
          icon =
            item.leaderboard.leaderboardCurrencyAddressesByNetwork[0]
              .currencyAddressesByNetwork.currency.logo;
        }
        items.push({
          _id: item._id,
          name: item.name,
  ...
  async function findCurrenciesForMenu(
    product: any,
    organization: any,
    req: any,
    isFromOrganizationAdminPath: boolean
    ...
    let data = await db.Currencies.find(filter)
      .populate({
        path: "currencyAddressesByNetwork",
        populate: {
          path: "networkDex",
